...
### Persistence details
- D1: **per-PR** database `<name>-db-pr-<N>` — created + migrated by CI on PR open and every push, deleted on PR close by `preview-cleanup.yml`. `bun run teardown` additionally sweeps any orphaned `-db-pr-*` databases (via `wrangler d1 list --json`). The shared `<name>-db-preview` still exists for manual preview work (`bun run db:migrate:preview`, `deploy:preview`).
...
| `.github/workflows/preview.yml` | CI: provision per-PR D1, migrate it, build with `CLOUDFLARE_ENV=preview`, `wrangler versions upload --preview-alias pr-<N>`, sticky PR comment |
